            ...
            try {
                ...
                switch (type) { 
                    ...
                    case 2:
                        ...
                        break;
                    case 3: //移动端 首页数据
                        List<article> article_read = new List<Mykisskui.article>();
                        List<article> article_new = new List<Mykisskui.article>();
                        List<article> article_new_top = new List<Mykisskui.article>();
                        article_read = article.OrderByDescending(f => f.read).Take(10).ToList();
                        article_new = article.OrderByDescending(f => f.Time).Take(10).ToList();
                        article = null;
                        foreach (article item in article_read) {
                            article_new_top.Add(item);
                        }
                        foreach (article item in article_new) {
                            article_new_top.Add(item);
                        }
                        article = article_new_top;
                        break;
                }
            }
            catch { }
